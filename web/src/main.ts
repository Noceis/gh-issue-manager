...
function renderColumn(column: ColumnData): HTMLElement {
...
function renderCard(card: CardData, colName: string, index: number): HTMLElement {
  ...
  const handleEl = el("button", "card-handle") as HTMLButtonElement;
  ...
  if (card.meta.due) {
    ...
    if (diff < 0) badge.classList.add("overdue");
    else if (diff <= 2) badge.classList.add("soon");
    else if (diff <= 7) badge.classList.add("upcoming");
...
async function handleDrop(evt: Sortable.SortableEvent) {
...
function syncIndices() {
  document.querySelectorAll<HTMLElement>(".cards").forEach((list) => {
    const colName = list.dataset.column!;
    list.querySelectorAll<HTMLElement>(".card").forEach((card, i) => {
...
function openModal(colName: string, cardIndex?: number) {
  ...
  // ── Form ──
  const form = el("form", "modal-form") as HTMLFormElement;
  form.innerHTML = `
    <label>
      Title
  ...
  if (isEdit) {
    const delBtn = el("button", "btn btn-danger") as HTMLButtonElement;
    delBtn.type = "button";
  ...
  const cancelBtn = el("button", "btn btn-secondary") as HTMLButtonElement;
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (due) meta.due = due;
    else delete meta.due;
    if (body && !meta.defaultExpanded) meta.defaultExpanded = "false";

    if (isEdit) {
      const newCol = (fd.get("column") as string) || colName;
